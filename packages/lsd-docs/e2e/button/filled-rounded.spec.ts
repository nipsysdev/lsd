...
test.describe('Button - Filled Rounded Example', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/examples/button/filled-rounded');
  });
  ...
  test('text button has correct background color', async ({ page }) => {
    const button = page.getByRole('button', { name: 'Add Item' });
  ...
  test('icon button has correct background color', async ({ page }) => {
    const iconButtons = page.getByRole('button').filter({ hasText: /^$/ });
    const button = iconButtons.nth(0);
